    ...
    ui(new Ui::result)
{
    ...
    connect(this,SIGNAL(refresh()),this,SLOT(show_result()));
	...
	if(q.exec()){
		...
		for(QString ei:ei_list){
			...
			for(QString apt : apt_list){
				...
				from eiinfo,goodsinfo,contact 
				where eiinfo.contact = goodsinfo.contact and goodsinfo.contact = contact.contact
				and ei="进货" and apartment="轮胎厂" ;
				*/
				sql = "select eiinfo.goodsname,eiinfo.contact,source,place,number,price,catogory,tel"
					" from eiinfo,goodsinfo,contact "
					" where eiinfo.contact = goodsinfo.contact and goodsinfo.contact = contact.contact"
				" and ei= :ei and apartment = :apt";
				q.prepare(sql);
				q.bindValue(":apt",apt);
				q.bindValue(":ei",ei);
				bool flag = true;
